...
def whole_step_forward(net, x):
    ...
    return torch.zeros((), device=x.coords.device), MLP_dairy, GT_dairy
...
def pack_mlp_cache(MLP_ls, GT_ls):
    """
    Merge per-depth MLP features into four stage-wise batches.
    The tune objective and number of optimizer steps stay unchanged.
    """
    feats_by_stage = [[] for _ in range(4)]
    gt_by_stage = [[] for _ in range(4)]

    for MLP_dairy, GT_dairy in zip(MLP_ls, GT_ls):
        for depth in sorted(MLP_dairy.keys()):
            if len(MLP_dairy[depth]) != 4 or len(GT_dairy[depth]) != 4:
                continue
            for stage in range(4):
                feats_by_stage[stage].append(MLP_dairy[depth][stage].detach())
                gt_by_stage[stage].append(GT_dairy[depth][stage].reshape(-1).long())

    packed_feats = tuple(torch.cat(feats, dim=0) for feats in feats_by_stage)
    packed_gt = tuple(torch.cat(gt, dim=0) for gt in gt_by_stage)
    return packed_feats, packed_gt


def only_mlp_forward_packed(net, packed_feats, packed_gt):
    heads = (
        net.group1_pred_head_s0,
        net.group1_pred_head_s1,
        net.group2_pred_head_s0,
        net.group2_pred_head_s1,
    )
    total_bits = packed_feats[0].new_zeros(())
    for head, feats, gt in zip(heads, packed_feats, packed_gt):
        selected_prob = head(feats).gather(1, gt.view(-1, 1))
        total_bits = total_bits + torch.sum(
            torch.clamp(-1.0 * torch.log2(selected_prob + 1e-10), 0, 50)
        )
    return total_bits
...
def compress_net(lossless_net, compress_ls, save_path='weights.bin'):
    ...
    for full_param_name, param in lossless_net.named_parameters():
        ...
        if in_postrain_module and is_target_layer:
            decoded = decoder.decodeWeights()
            decoded = torch.as_tensor(decoded, device=param.device, dtype=param.dtype).reshape(param.shape)
...
def init_net(net, x, pre_ckpt_path, postrain_ls, lr, save_path='weights.bin', max_epoch=800, tune=True):
    ...
    with torch.no_grad():
        ...
        # Pack each chunk immediately to avoid per-depth work in every epoch.
        packed_cache_ls = []
        for chunk_x in chunks:
            bpp, MLP_dairy, GT_dairy = whole_step_forward(net, chunk_x)
            packed_feats, packed_gt = pack_mlp_cache([MLP_dairy], [GT_dairy])
            packed_cache_ls.append((packed_feats, packed_gt))
    ...
    try:
        optimizer = torch.optim.Adam(trainable_params, lr=lr, foreach=True)
    except TypeError:
    ...
    with torch.enable_grad():
        for epoch in range(max_epoch):
            for packed_feats, packed_gt in packed_cache_ls:
                # Zero gradients
                optimizer.zero_grad(set_to_none=True)
                
                # Forward pass (only through MLP heads)
                total_bits = only_mlp_forward_packed(net, packed_feats, packed_gt)
                bpp = total_bits / N
