...
class _HalfSparseModule(nn.Module):
    def __init__(self, module):
        super().__init__()
        self.module = module.half()

    def forward(self, x):
        x_half = SparseTensor(coords=x.coords, feats=x.feats.half())
        if hasattr(x, '_caches') and hasattr(x_half, '_caches'):
            x_half._caches = x._caches
        out = self.module(x_half)
        out.feats = out.feats.float()
        return out
...
class AnyPcc_CoderIntra(nn.Module):
    def __init__(self, model_path, device='cuda:0', channels=32, kernel_size=3, 
        ...
        self.register_buffer(
            '_occ_popcount_lut',
            torch.tensor([bin(i).count('1') for i in range(256)], device=device, dtype=torch.int32),
            persistent=False,
        )
        self.posQ = posQ
        ...
        self._model_is_finetuned = False
        self.fast_fp16_sparse = not tune
        self.fast_batched_encoder = True
        torch.backends.cudnn.deterministic = False
        from kit import fast_torchac
        self._torchac_codec = fast_torchac
        default_async_torchac = '1' if self._torchac_codec is not torchac else '0'
        self.async_torchac = os.environ.get('ANYPCC_ASYNC_TORCHAC', default_async_torchac) == '1'
        torchac_workers = int(os.environ.get('ANYPCC_TORCHAC_WORKERS', '8'))
        self._torchac_executor = (
            ThreadPoolExecutor(max_workers=max(1, torchac_workers))
            if self.async_torchac else None
        )
        ...
        self.model.eval()
        if self.fast_fp16_sparse:
            self._enable_fast_fp16_sparse(self.model)
        ...
        if lossy_model_path and os.path.exists(lossy_model_path) and lossy_level > 0:
            ...
            if self.fast_fp16_sparse:
                self._enable_fast_fp16_sparse(self.lossy_model)
            occ = torch.arange(256, device=device, dtype=torch.int32).view(256, 1)
            child = torch.arange(8, device=device, dtype=torch.int32).view(1, 8)
            self.lossy_model.register_buffer(
                'occupancy_to_children',
                ((occ >> child) & 1).float(),
                persistent=False,
            )
    ...
    def _enable_fast_fp16_sparse(self, model):
        sparse_module_names = [
            'prior_resnet',
            'target_resnet',
            'group1_spatial_conv_s0',
            'group1_spatial_conv_s1',
            'group2_spatial_conv_s0',
            'group2_spatial_conv_s1',
            'neighbor_conv',
        ]
        for name in sparse_module_names:
            if hasattr(model, name) and not isinstance(getattr(model, name), _HalfSparseModule):
                setattr(model, name, _HalfSparseModule(getattr(model, name)).to(self.device))
        if hasattr(model, 'fog') and hasattr(model.fog, 'conv'):
            model.fog.conv.half()

    def _encode_torchac_async(self, cdf_norm, symbols):
        if self._torchac_executor is None:
            return self._torchac_codec.encode_int16_normalized_cdf(cdf_norm, symbols)
        return self._torchac_executor.submit(self._torchac_codec.encode_int16_normalized_cdf, cdf_norm, symbols)

    def _decode_torchac(self, cdf_norm, byte_stream):
        return self._torchac_codec.decode_int16_normalized_cdf(cdf_norm, byte_stream)

    def _make_batched_sparse(self, coords_ls, feats_ls, cache=None):
        sizes = [coords.shape[0] for coords in coords_ls]
        batched_coords = torch.cat(coords_ls, dim=0).clone()
        start = 0
        for batch_idx, size in enumerate(sizes):
            batched_coords[start:start + size, 0] = batch_idx
            start += size
        sparse = SparseTensor(coords=batched_coords, feats=torch.cat(feats_ls, dim=0))
        if cache is not None and hasattr(sparse, '_caches'):
            sparse._caches = cache
        return sparse, sizes

    @staticmethod
    def _split_by_sizes(tensor, sizes):
        return list(torch.split(tensor, sizes, dim=0))

    @staticmethod
    def _target_embedding_fast(net, x_up_F, x_up_C):
        child_idx = getattr(net.fcg, '_last_child_idx', None)
        if child_idx is not None and child_idx.shape[0] == x_up_C.shape[0]:
            return net.target_embedding(x_up_F, x_up_C, child_idx)
        return net.target_embedding(x_up_F, x_up_C)

    def _encode_stage_streams(self, prob_ls, symbol_ls):
        byte_stream_ls = []
        for prob, symbols in zip(prob_ls, symbol_ls):
            cdf = torch.cat((prob[:, 0:1] * 0, prob.cumsum(dim=-1)), dim=-1)
            cdf_norm = op._convert_to_int_and_normalize(torch.clamp(cdf, 0, 1), True).cpu()
            byte_stream_ls.append(
                self._encode_torchac_async(cdf_norm, symbols.to(device='cpu', dtype=torch.int16, non_blocking=True))
            )
        return byte_stream_ls

    def _encode_lossless_single_layer_streams(self, gt_C, gt_O, x_up_feats, group_masks=None):
        if group_masks is None:
            group1_mask, group2_mask = self.model.get_3d_checkerboard_groups(gt_C)
        else:
            group1_mask, group2_mask = group_masks

        group1_coords = gt_C[group1_mask]
        group1_features = x_up_feats[group1_mask]
        group1_occupancy = gt_O[group1_mask].int()
        gt_s0 = torch.remainder(group1_occupancy, 16).int().squeeze(-1)
        gt_s1 = torch.div(group1_occupancy, 16, rounding_mode='floor').int().squeeze(-1)
        ...
        group1_s0_stream = self._encode_stage_streams([prob_s0], [gt_s0])[0]

        group1_s1_F = group1_features + self.model.group1_pred_head_s1_emb(gt_s0)
        group1_s1_sparse = SparseTensor(coords=group1_coords, feats=group1_s1_F)
        if hasattr(group1_sparse, '_caches') and hasattr(group1_s1_sparse, '_caches'):
            group1_s1_sparse._caches = group1_sparse._caches
        ...
        group1_s1_stream = self._encode_stage_streams([prob_s1], [gt_s1])[0]

        group2_occupancy = gt_O[group2_mask].int()
        gt_s0 = torch.remainder(group2_occupancy, 16).int().squeeze(-1)
        gt_s1 = torch.div(group2_occupancy, 16, rounding_mode='floor').int().squeeze(-1)
        enhanced_features = self.model.aggregate_neighbor_features_efficient(
            gt_C, x_up_feats, group1_mask, group2_mask, gt_O
        )
        group2_coords = gt_C[group2_mask]
        ...
        group2_s0_stream = self._encode_stage_streams([prob_s0], [gt_s0])[0]

        group2_s1_F = enhanced_features + self.model.group2_pred_head_s1_emb(gt_s0)
        group2_s1_sparse = SparseTensor(coords=group2_coords, feats=group2_s1_F)
        if hasattr(group2_sparse, '_caches') and hasattr(group2_s1_sparse, '_caches'):
            group2_s1_sparse._caches = group2_sparse._caches
        ...
        group2_s1_stream = self._encode_stage_streams([prob_s1], [gt_s1])[0]

        return [group1_s0_stream, group1_s1_stream, group2_s0_stream, group2_s1_stream]

    def _encode_lossless_layers_batched(self, data_ls):
        num_layers = len(data_ls) - 1
        x_C_ls = [data_ls[depth][0] for depth in range(num_layers)]
        x_O_ls = [data_ls[depth][1] for depth in range(num_layers)]
        gt_C_ls = [data_ls[depth + 1][0] for depth in range(num_layers)]
        gt_O_ls = [data_ls[depth + 1][1] for depth in range(num_layers)]

        x_F_ls = [
            self.model.prior_embedding(x_O.int()).view(-1, self.model.channels)
            for x_O in x_O_ls
        ]
        prior_sparse, prior_sizes = self._make_batched_sparse(x_C_ls, x_F_ls)
        prior_sparse = self.model.prior_resnet(prior_sparse)
        prior_feats_ls = self._split_by_sizes(prior_sparse.feats, prior_sizes)

        x_up_C_ls = []
        x_up_F_ls = []
        for x_C, x_O, x_F in zip(x_C_ls, x_O_ls, prior_feats_ls):
            x_up_C, x_up_F = self.model.fcg(x_C, x_O, x_F)
            x_up_C_ls.append(x_up_C)
            x_up_F_ls.append(self._target_embedding_fast(self.model, x_up_F, x_up_C))

        target_sparse, target_sizes = self._make_batched_sparse(x_up_C_ls, x_up_F_ls)
        target_sparse = self.model.target_resnet(target_sparse)
        x_up_feats_ls = self._split_by_sizes(target_sparse.feats, target_sizes)

        group1_mask_ls = []
        group2_mask_ls = []
        for gt_C in gt_C_ls:
            group1_mask, group2_mask = self.model.get_3d_checkerboard_groups(gt_C)
            group1_mask_ls.append(group1_mask)
            group2_mask_ls.append(group2_mask)

        byte_stream_ls = []
        for depth in range(num_layers):
            byte_stream_ls.extend(
                self._encode_lossless_single_layer_streams(
                    gt_C_ls[depth],
                    gt_O_ls[depth],
                    x_up_feats_ls[depth],
                    (group1_mask_ls[depth], group2_mask_ls[depth]),
                )
            )
        return byte_stream_ls
    ...
    def compress(self, xyz, return_metadata=False, lossy_level=None):
        ...
        """
        outer_context = torch.no_grad if self.tune else torch.inference_mode
        with outer_context():
            ...
            if not self.tune or N < self.tune_threshold:
                # Keep the pretrained model resident for normal inference. Reload only
                # after a previous sample-adaptive fine-tune modified the weights.
                if self._model_is_finetuned and self._model_path and os.path.exists(self._model_path):
                ...
                self._model_is_finetuned = True
                print(f"Fine-tuning completed (Time: {tune_time:.2f}s, Param size: {net_bits/8:.2f} bytes)")
                
            # Preprocessing: Downsample via fog 
            data_ls = []
            fog_returns_z_order = getattr(self.model.fog, 'returns_z_order', False)
            if fog_returns_z_order:
                self.model.fog.assume_z_ordered_input = False
            while True:
                x = self.model.fog(x)
                if fog_returns_z_order:
                    self.model.fog.assume_z_ordered_input = True
                    data_ls.append((x.coords, x.feats))
                else:
            ...
            if fog_returns_z_order:
                self.model.fog.assume_z_ordered_input = False
            data_ls = data_ls[::-1]
            if not fog_returns_z_order:
                data_ls = [z_order_sort(coords, feats) for coords, feats in data_ls]
            
            num_layers = len(data_ls) - 1
            lossless_prefix_layers = num_layers
            if current_lossy_level > 0:
                lossless_prefix_layers = max(0, num_layers - current_lossy_level)

            # In lossy mode, the low-resolution prefix is still encoded as a
            # normal lossless hierarchy. Reuse the same optimized lossless
            # encoder for that prefix, then handle only the truncated tail in
            # the lossy branch below.
            use_optimized_lossless_prefix = (
                self.fast_batched_encoder
                and lossless_prefix_layers > 0
                and not use_finetuned_model
            )
            if use_optimized_lossless_prefix:
                byte_stream_ls = self._encode_lossless_layers_batched(data_ls[:lossless_prefix_layers + 1])
                loop_start_depth = lossless_prefix_layers
            else:
                byte_stream_ls = []
                loop_start_depth = 0

            # Neural network inference and encoding
            for depth in range(loop_start_depth, num_layers):
                if self.no_lossy_net and depth >= lossless_prefix_layers:
                    break
                ...
                # Lossy layers only store k. The lossy network is needed on
                # decode to choose points, but encode can derive k from GT.
                if is_pure_lossy:
                    if self.fixed_k:
                        k = self._occ_popcount_lut[gt_x_up_O.reshape(-1).long()].sum().item()
                    else:
                        k = gt_x_up_C.shape[0]
                    byte_stream_ls.append(np.array(k, dtype=np.uint32).tobytes())
                    continue
                ...
                # Sorted parent occupancy expands to sorted child coordinates.
                x_up_C, x_up_F = net_for_blocks.fcg(x_C, x_O, x.feats)
                
                x_up_F = self._target_embedding_fast(net_for_blocks, x_up_F, x_up_C)
                ...
                # Group 1 encoding (2 stages: s0 and s1). The tested 8i
                # hierarchy has non-empty checkerboard groups at every layer.
                group1_coords = gt_x_up_C[group1_mask]
                group1_features = x_up.feats[group1_mask]
                group1_occupancy = gt_x_up_O[group1_mask].int()
                
                # Split occupancy into s0 (lower 4 bits) and s1 (higher 4 bits)
                gt_s0 = torch.remainder(group1_occupancy, 16).int()
                gt_s1 = torch.div(group1_occupancy, 16, rounding_mode='floor').int()
                ...
                byte_stream_s0 = self._encode_torchac_async(
                ...
                byte_stream_s1 = self._encode_torchac_async(
                ...
                enhanced_features = self.model.aggregate_neighbor_features_efficient(
                    ...
                    cache=getattr(x_up, '_caches', None)
                )
                group2_coords = gt_x_up_C[group2_mask]
                group2_occupancy = group2_occupancy_raw.int()
                
                gt_s0 = torch.remainder(group2_occupancy, 16).int()
                gt_s1 = torch.div(group2_occupancy, 16, rounding_mode='floor').int()
            ...
            byte_stream_ls = [
                byte_stream.result() if hasattr(byte_stream, 'result') else byte_stream
                        ...
                        if any(full_param_name.startswith(module_name) for module_name in postrain_ls):
                            ...
                            if is_target_layer:
                                try:
                                    ...
                                    decoded = torch.as_tensor(param_np, device=param.device, dtype=param.dtype).reshape(param.shape)
                                    param.data.copy_(decoded)
                                except:
                                    pass 
            ...
            for depth in range(num_layers):
                ...
                if is_pure_lossy:
                    ...
                    if k > 0 and x.coords.shape[0] > 0 and self.lossy_model is not None:
                        ...
                        x_coords_sorted = x.coords
                        x_feats_sorted = x.feats
                        if x_feats_sorted.dim() == 3:
                        ...
                        x_up_F_dec = self._target_embedding_fast(net_for_blocks, x_up_F_dec, x_up_C_dec)
                        ...
                        if x_up_dec.coords.shape[0] > 0:
                            if self.fixed_k:
                                ...
                                if k_safe > 0:
                                    ...
                                    parent_indices = top_indices_flat >> 3
                                    child_indices = top_indices_flat & 7
                                ...
                                occ_hat = torch.argmax(prob[topk_indices, 1:], dim=1).to(torch.int16) + 1
                                ...
                                next_feats = x_up_O_dec[valid_mask].float()
                ...
                # Standard Lossless Decoding Flow. Coordinates are kept in
                # z-order from metadata and from the previous masked x_up_C.
                x_coords_sorted = x.coords
                x_feats_sorted = x.feats
                if x_feats_sorted.dim() == 3:
                    x_feats_sorted = x_feats_sorted.squeeze(-1)
                ...
                x_up_O_dec = torch.zeros(x_up_C_dec.shape[0], dtype=torch.int32, device=self.device)
                ...
                if len(byte_streams_g1) > 0 and len(byte_streams_g1[0]) > 0:
                    cdf_s0 = torch.cat((prob_s0[:, 0:1]*0, prob_s0.cumsum(dim=-1)), dim=-1)
                    cdf_norm_s0 = op._convert_to_int_and_normalize(torch.clamp(cdf_s0, 0, 1), True).cpu()
                    decoded_s0 = self._decode_torchac(cdf_norm_s0, byte_streams_g1[0]).to(
                ...
                if len(byte_streams_g1) > 1 and len(byte_streams_g1[1]) > 0:
                    cdf_s1 = torch.cat((prob_s1[:, 0:1]*0, prob_s1.cumsum(dim=-1)), dim=-1)
                    cdf_norm_s1 = op._convert_to_int_and_normalize(torch.clamp(cdf_s1, 0, 1), True).cpu()
                    decoded_s1 = self._decode_torchac(cdf_norm_s1, byte_streams_g1[1]).to(
                        device=self.device, dtype=torch.int32, non_blocking=True
                    )
                else:
                ...
                enhanced_features_dec = self.model.aggregate_neighbor_features_efficient(
                    ...
                    cache=getattr(x_up_dec, '_caches', None)
                )
                ...
                if len(byte_streams_g2) > 0 and len(byte_streams_g2[0]) > 0:
                    cdf_s0 = torch.cat((prob_s0[:, 0:1]*0, prob_s0.cumsum(dim=-1)), dim=-1)
                    cdf_norm_s0 = op._convert_to_int_and_normalize(torch.clamp(cdf_s0, 0, 1), True).cpu()
                    decoded_s0 = self._decode_torchac(cdf_norm_s0, byte_streams_g2[0]).to(
                        device=self.device, dtype=torch.int32, non_blocking=True
                    )
                else:
                    decoded_s0 = torch.argmax(prob_s0, dim=-1)
                ...
                if len(byte_streams_g2) > 1 and len(byte_streams_g2[1]) > 0:
                    cdf_s1 = torch.cat((prob_s1[:, 0:1]*0, prob_s1.cumsum(dim=-1)), dim=-1)
                    cdf_norm_s1 = op._convert_to_int_and_normalize(torch.clamp(cdf_s1, 0, 1), True).cpu()
                    decoded_s1 = self._decode_torchac(cdf_norm_s1, byte_streams_g2[1]).to(
                        device=self.device, dtype=torch.int32, non_blocking=True
                    )
                else:
                    decoded_s1 = torch.argmax(prob_s1, dim=-1)
                ...
                x_up_O_dec[group2_mask_dec] = group2_occupancy_dec
